...
a release can bump pyproject.toml's [project].version while leaving
__init__.py's __version__ stale, so the wheel's package metadata and
the CLI's own --version output disagree. Applied here proactively
before it ever happens, not after.
"""
...
from qdrift import __version__
